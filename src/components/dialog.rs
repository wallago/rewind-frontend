...
pub fn DialogTrigger(props: DialogProps) -> Element {
    ...
    if props.as_child.unwrap_or(false) {
        ...
        rsx!(
            Button { onclick: Some(EventHandler::new(move |_| { ctx.is_open.set(true) })),
                {props.children}
            }
        )
    }
...
pub fn DialogClose(props: DialogProps) -> Element {
    ...
    if props.as_child.unwrap_or(false) {
        rsx!(
            div {
                id: props.id.clone().unwrap_or_default(),
                onclick: move |_| { ctx.is_open.set(false) },
                {props.children}
            }
        )
    } else {
        rsx!(
            Button {
                onclick: Some(EventHandler::new(move |_| { ctx.is_open.set(false) })),
                class: "px-2 text-sm",
                "Cancel"
            }
        )
...
pub fn DialogDescription(props: DialogProps) -> Element {
    rsx!(
        p { class: "text-sm font-medium text-secondary-2", {props.children} }
    )
}
...
pub fn DialogFooter(props: DialogProps) -> Element {
    rsx!(
        div { class: "mt-2 flex justify-end gap-4", {props.children} }
    )
}
...
pub fn DialogTitle(props: DialogProps) -> Element {
    rsx!(
        h2 { class: "text-lg font-semibold", {props.children} }
    )
}
